...
public class GraphicObject
{
    ...
    private IEnumerator Fading(float target, float speed, Texture blend) {
        ...
        while (renderer.material.GetFloat(opacityParam) != target) {
            float opacity = Mathf.MoveTowards(renderer.material.GetFloat(opacityParam), target, speed * Time.deltaTime); // TODO ptet ajouter en multiplier du speed le default transition speed
